...
def getCatmullRomTranslation(time, ctrlPoint):
    t = np.array([time ** 3, time ** 2, time, 1])
    
    p = []
    for i in range(0, 4):
        pos = getTranslation(ctrlPoints[(ctrlPoint + numCtrlPoint - 1 + i) % numCtrlPoint])
        p.append(pos)
    ...
    hermite = np.array([[2, -2, 1, 1],
                        ...
                        [0, 0, 1, 0],
                        [1, 0, 0, 0]])
    catmullRomTangent = np.array([[0, 1, 0, 0],
                                  [0, 0, 1, 0],
                                  [-.5, 0, .5, 0],
                                  [0, -.5, 0, .5]])
    catmullRom = hermite @ catmullRomTangent
    
    return t.T @ catmullRom @ p

def getEulerRotation(prvPosition, curPosition):
  x1, y1, z1 = prvPosition.T
  x2, y2, z2 = curPosition.T

  v = [x2 - x1, y2 - y1, z2 - z1]

  roll = np.identity(3)

  pitchAngle = np.arctan2(v[1], np.sqrt((v[0]) ** 2 + (v[2]) ** 2))
  pitch = np.array([[np.cos(pitchAngle), -np.sin(pitchAngle), 0],
                     [np.sin(pitchAngle), np.cos(pitchAngle), 0],
                     [0, 0, 1]])

  yawAngle = np.arctan2(v[0], v[2]) - np.pi/2
  yaw = np.array([[np.cos(yawAngle), 0, np.sin(yawAngle)],
                   [0, 1, 0],
                   [-np.sin(yawAngle), 0, np.cos(yawAngle)]])

  return yaw @ pitch @ roll

def display():
    global cameraIndex, cow2wld, ctrlPoints, isRiding, curPoint, animStartTime
    ...
    if isRiding :
        ...
        intAnimTime = int(animTime)

        if animTime < 18:
            M = np.eye(4)
            setTranslation(M, getCatmullRomTranslation(animTime - intAnimTime, intAnimTime))
            setRotation(M, getEulerRotation(getTranslation(cow2wld), getTranslation(M)))
            cow2wld = M
        else:
            isRiding = 0
            ctrlPoints = []
...
def initialize(window):
    global cursorOnCowBoundingBox, floorTexID, cameraIndex, camModel, cow2wld, cowModel, startState
    cursorOnCowBoundingBox=False;
    # Set up OpenGL state
...
def onMouseButton(window,button, state, mods):
    global isDrag, V_DRAG, H_DRAG, cow2wld, cursorOnCowBoundingBox
    global ctrlPoints, isRiding, animStartTime
    if isRiding:
        return
    GLFW_DOWN=1;
    GLFW_UP=0;
    ...
    if button == glfw.MOUSE_BUTTON_LEFT:
        ...
        if state == GLFW_DOWN:
            ...
            if len(ctrlPoints) > 0 and np.array_equal(ctrlPoints[len(ctrlPoints) - 1], cow2wld):
                return
            ctrlPoints.append(cow2wld)
            if len(ctrlPoints) == numCtrlPoint:
                isDrag = 0
...
def onMouseDrag(window, x, y):
    global isDrag,cursorOnCowBoundingBox, pickInfo, cow2wld, isRiding
    if isRiding:
        return
    if isDrag:
        ...
        if  isDrag==V_DRAG:
            ...
            if cursorOnCowBoundingBox:
                ...
                pp=pickInfo;
                n = np.cross(ray.direction, np.array((0, 1, 0)))
                n = np.cross(n, np.array((0, 1, 0)))
                p=Plane(n, pp.cowPickPosition);
                ...
                T=np.eye(4)
                setTranslation(T, np.array((0, currentPos[1] - pp.cowPickPosition[1], 0)))
